...
import { GraduationCap, Eye, EyeOff, Loader2, Mail, Lock, Users, BookOpen, Shield, TrendingUp, Star, ArrowRight, UserPlus } from "lucide-react"
import Link from "next/link"

const roles = [
  { id: "staff", label: "Staff / Admin", icon: Shield },
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center bg-slate-50 p-6 sm:p-8 lg:p-12">
        <div className="w-full max-w-[420px] space-y-8">
          ...
          {/* Role Switcher */}
          <div className="bg-white rounded-xl border border-slate-200/80 p-1 flex shadow-sm">
            {roles.map(r => (
              <button key={r.id} onClick={() => setRole(r.id)}
                className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                  role === r.id ? "bg-indigo-600 text-white shadow-md shadow-indigo-200" : "text-gray-500 hover:text-gray-700 hover:bg-gray-50"
                }`}>
          ...
          {/* Google */}
          <button className="w-full py-3 bg-white border border-slate-200/80 rounded-xl text-gray-700 font-medium text-sm hover:bg-gray-50 shadow-sm transition-all flex items-center justify-center gap-3">
