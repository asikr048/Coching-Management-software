    ...
    async function loadData() {
      const fetchAuth = async () => {
        try {
          ...
          if (user) {
            setCurrentUser(user)
            let { data: staff } = await supabase.from("staff").select("role").eq("auth_user_id", user.id).maybeSingle()
            if (!staff && user.email) {
              const { data: staffByEmail } = await supabase.from("staff").select("role").eq("email", user.email).maybeSingle()
              if (staffByEmail) staff = staffByEmail
            }
            let pRole = null
            try {
            ...
            setUserRole(staff?.role || pRole || (user.user_metadata?.role) || "student")
