...
export default function SiteContentPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      <h1 className="text-2xl font-black text-slate-900 tracking-tight">Site Content Management</h1>
      ...
      <div className="flex gap-2 border-b border-slate-200 pb-1">
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id as any)}
            className={`flex items-center gap-2 px-4 py-2 rounded-t-xl text-sm font-semibold border-b-2 transition-all cursor-pointer ${tab === t.id ? 'border-amber-400 text-amber-400 bg-amber-500/10' : 'border-transparent text-slate-400 hover:text-slate-200 hover:bg-amber-50/30'}`}>
      ...
      {tab === 'notices' && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-6 space-y-4">
            <h2 className="font-bold text-slate-900 flex items-center gap-2"><Plus className="w-4 h-4 text-amber-400" /> Post New Notice</h2>
            <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Notice title" className="w-full px-4 py-3 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 shadow-2xs transition-all" />
            <textarea value={newContent} onChange={e => setNewContent(e.target.value)} placeholder="Notice content..." rows={3} className="w-full px-4 py-3 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 shadow-2xs transition-all resize-none" />
            <div className="flex items-center justify-between gap-4">
              <select value={newPriority} onChange={e => setNewPriority(e.target.value)} className="px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-500/10">
              ...
              <button onClick={addNotice} disabled={addingNotice} className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-black rounded-xl transition-all shadow-lg shadow-amber-500/20 disabled:opacity-50 text-sm cursor-pointer">
          ...
          <div className="space-y-3">
            {loadingNotices ? (
              ...
              <div key={n.id} className={`bg-white rounded-2xl border p-4 flex items-start justify-between gap-4 shadow-md transition-all ${n.is_active ? 'border-slate-200' : 'border-dashed border-slate-200 opacity-60'}`}>
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => toggleNotice(n.id, n.is_active)} className="p-2 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-800 text-slate-400 hover:text-white transition-colors cursor-pointer" title={n.is_active ? 'Hide' : 'Show'}>
      ...
      {tab === 'feedback' && (
        <div className="space-y-3">
          {loadingFeedback ? (
            ...
            <div key={f.id} className={`bg-white rounded-2xl border p-5 space-y-2 transition-all ${f.is_read ? 'border-slate-200 opacity-75' : 'border-amber-500/30 shadow-lg shadow-amber-500/5'}`}>
              ...
              <p className="text-sm text-slate-300 bg-white border border-slate-300/80 rounded-xl p-3">{f.message}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {/* SETTINGS TAB */}
      {tab === 'settings' && (
        <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-6 space-y-5 max-w-lg">
          <h2 className="font-bold text-slate-900 flex items-center gap-2"><Link2 className="w-4 h-4 text-amber-400" /> Contact / Message Link</h2>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Button Label</label>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Link URL</label>
          ...
          <button onClick={saveSettings} disabled={savingSettings} className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-black rounded-xl transition-all shadow-lg shadow-amber-500/20 disabled:opacity-50 text-sm cursor-pointer">
