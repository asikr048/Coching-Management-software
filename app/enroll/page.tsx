...
function EnrollContent() {
  ...
  const [coachingPhone, setCoachingPhone] = useState("01302201431")
  ...
  // Load batches and site settings
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        // Fetch coaching phone from settings
        ...
        if (settingData?.value) {
          setCoachingPhone(settingData.value)
        }

        // Check if user is logged in — auto-fill info if student
        ...
        if (user) {
          ...
          if (student) {
            ...
            // If a batch is already selected, check enrollment or pending status
  ...
  const monthlyFee = Number(selectedBatch?.monthly_fee || 0)
  const admissionFee = Number(selectedBatch?.admission_fee || 0)
  ...
  async function handleSubmitPayment(e: React.FormEvent) {
    ...
    try {
      ...
      if (!stDbId) {
        ...
        // Update existing student details with new submission info
        await supabase
          .from("students")
          .update({
            name: form.name.trim(),
            ...
            school_college: form.school_college.trim() || undefined,
            class_level: form.class_level.trim() || undefined,
            address: form.address.trim() || undefined,
            referred_by_code: form.referred_by_code.trim() || undefined,
          })
          .eq("id", stDbId)
      }
      ...
      // 3. Check for already active enrollment in this batch
      ...
      // 4. Check for duplicate pending payment submission for this batch
      ...
      // 5. Insert payment submission record
      ...
      window.scrollTo({ top: 0, behavior: "smooth" })
      setStep("success")
      toast.success("Payment submitted successfully! Admin will verify and activate your enrollment.")
    } catch (err: unknown) {
  ...
  function handleCopyPhone() {
    navigator.clipboard.writeText(coachingPhone)
    setCopiedPhone(true)
    toast.success("Coaching phone number copied!")
        ...
        <div className="max-w-2xl mx-auto w-full pt-6 pb-12">
          ...
          <div className="bg-[#0f172a] border border-[#1e293b] rounded-3xl p-8 md:p-10 shadow-2xl relative overflow-hidden text-center">
            ...
            <p className="text-gray-400 text-sm max-w-md mx-auto mb-6">
              {alreadyEnrolled
                ? "You have already completed enrollment for this batch. You can access your classes from your profile."
            ...
            <div className="bg-[#111a2e] border border-[#1b2a47] rounded-xl p-4 text-left text-xs space-y-2 mb-8 max-w-md mx-auto">
              ...
              <div className="flex justify-between py-1 border-b border-[#1c2c4a]">
                ...
                <span className="font-semibold text-cyan-300">{selectedBatch?.name}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-[#1c2c4a]">
        ...
        <main className="max-w-5xl mx-auto w-full px-4 py-8 md:py-12 flex-1">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              {/* PAY VIA row */}
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">PAY VIA</p>
                <div className="flex flex-wrap gap-2.5">
                  <div className="px-4 py-2 rounded-lg bg-[#e2136e]/15 text-[#ff4b94] border border-[#e2136e]/40 font-bold text-xs">
                    bKash
                  </div>
                  <div className="px-4 py-2 rounded-lg bg-[#f05a22]/15 text-[#ff7a45] border border-[#f05a22]/40 font-bold text-xs">
                    Nagad
                  </div>
                  <div className="px-4 py-2 rounded-lg bg-[#8c3fa8]/15 text-[#ba68c8] border border-[#8c3fa8]/40 font-bold text-xs">
                    Rocket
                  </div>
                  <div className="px-4 py-2 rounded-lg bg-[#00a884]/15 text-[#20c997] border border-[#00a884]/40 font-bold text-xs">
                    Upay
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0e1728]/90 border border-[#1a2b47] rounded-2xl p-6 space-y-4 shadow-xl">
                <p className="text-xs font-bold text-[#00ffff] uppercase tracking-wider mb-1">HOW TO PAY</p>
                
                <div className="space-y-3.5 text-xs md:text-sm text-gray-300">
                  <div className="flex items-start gap-3">
                    <span className="text-base flex-shrink-0 mt-0.5">📱</span>
                    <p className="leading-relaxed">
                      Open <strong className="text-[#ff4b94]">bKash</strong>, <strong className="text-[#ff7a45]">Nagad</strong>, <strong className="text-[#ba68c8]">Rocket</strong>, or <strong className="text-[#20c997]">Upay</strong> App — or Dial on Phone
                    </p>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    <span className="text-base flex-shrink-0 mt-0.5">👉</span>
                    <p className="leading-relaxed">
                      Choose <strong className="text-[#00ffff] font-bold">&quot;Send Money&quot;</strong>
                    </p>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    <span className="text-base flex-shrink-0 mt-0.5">🔑</span>
                    <p className="leading-relaxed">
                      Enter your <strong className="text-cyan-300 font-semibold">PIN</strong> to confirm
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-[#182842] flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setStep("admission")}
                    className="text-xs text-cyan-400 hover:text-cyan-300 font-semibold hover:underline flex items-center gap-1 cursor-pointer"
                  >
                    Edit Details ✎
                  </button>
                </div>
            ...
            <div className="lg:col-span-5">
              <form onSubmit={handleSubmitPayment} className="bg-[#0e1728] border border-[#1a2b47] rounded-2xl p-6 md:p-8 space-y-6 shadow-2xl">
                ...
                <div>
                  ...
                  >
                    <option value="" disabled className="bg-[#0e1728] text-gray-400">Select method...</option>
                    <option value="bkash" className="bg-[#0e1728] text-white">bKash</option>
                    <option value="nagad" className="bg-[#0e1728] text-white">Nagad</option>
                    <option value="rocket" className="bg-[#0e1728] text-white">Rocket</option>
                    <option value="upay" className="bg-[#0e1728] text-white">Upay</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-[11px] text-gray-400 mt-1.5">Found in your bKash / Nagad confirmation SMS</p>
                </div>
      ...
      <nav className="bg-white/95 backdrop-blur-md border-b border-gray-100 sticky top-0 z-30 shadow-xs">
        <div className="max-w-5xl mx-auto px-4 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-indigo-700 bg-indigo-50 px-3.5 py-1.5 rounded-full border border-indigo-200">
            <span>Step 1: Admission Form</span>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-12">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-indigo-50 border border-indigo-200 text-indigo-700 text-xs font-bold px-4 py-1.5 rounded-full mb-3 uppercase tracking-wider">
            <Sparkles className="w-3.5 h-3.5" /> Batch Admission Form
          </div>
        ...
        <form onSubmit={handleProceedToPayment} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-gray-900">Enrolling Batch</h3>
                  <p className="text-xs text-gray-500">Select the batch program you are enrolling in</p>
                </div>
              </div>
            ...
            {selectedBatch && (
              <div className="mt-4 p-4 bg-gradient-to-r from-indigo-50/60 to-violet-50/60 rounded-xl border border-indigo-100/80 flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="text-right">
                  ...
                  {admissionFee > 0 && (
                    <p className="text-[11px] text-gray-400">Monthly ৳{monthlyFee} + Admission ৳{admissionFee}</p>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-indigo-600 to-violet-600 px-6 py-4 flex items-center gap-3">
              ...
              <h3 className="font-bold text-white text-base">Student Personal Information</h3>
            </div>
